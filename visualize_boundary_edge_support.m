...
% 3) projects all boundary points+tangents to ONE camera view
% 4) loads the detected 2D third-order edges for that view and buckets them
%    once into a uniform pixel grid (coarse-to-fine spatial partition) --
%    this is the SAME grid drawn in the visualization below
% 5) for each projected boundary point, only compares against 2D edges in
%    its own grid cell + the 8 neighboring cells (instead of all N edges),
%    and marks the point "supported" if a nearby edge also matches in
%    orientation
% 6) displays the grid, the 2D edges (green), and ALL boundary points
%    colored by support status (blue = supported, red = unsupported)

% ---- what to show ----
PARAMS.MESH_NAME = 'loftsurf_1_2_reverse.ply'; % mesh file inside blender/output
PARAMS.VIEW = 13;                              % camera view index (0-based, as in the data files)

% ---- sampling ----
PARAMS.MESH_DIR = fullfile(pwd, 'blender', 'output');
PARAMS.SAMPLES_PER_SURFACE = 300;
PARAMS.RESTRICT_TO_FREE_BOUNDARY = 0; % 0 = show/evaluate the whole boundary loop; 1 = only the free boundary
PARAMS.BOUNDARY_ASSIGNMENT_TOL = [];  % manual curve-assignment tolerance, or [] to derive from curve spacing (only used if RESTRICT_TO_FREE_BOUNDARY = 1)
...
PARAMS.TAU_ORIENTATION_DEG = 5;  % degrees; max orientation difference (mod 180) to count as support
...
% ---- detected 2D third-order edges for the selected view ----
...
% ---- input curves, needed to know which boundary samples are "free" ----
curve1 = [];
curve2 = [];
...
elseif PARAMS.RESTRICT_TO_FREE_BOUNDARY == 1
    fprintf('Warning: %s not found; cannot identify the free boundary, using the whole boundary loop instead.\n', preProcessedFile);
end

% ---- sample the mesh boundary (ordered loop) + labels ----
...
fprintf('  sampled %d boundary points\n', size(sampledPts, 1));

% ---- 3D tangent at every sampled point, using the full ordered loop ----
worldTangents = compute_world_tangents(sampledPts);

% ---- project points + tangents to the selected view ----
uv = project_points_to_image(sampledPts, projMatrix);
tangent2D = compute_camera_tangents(sampledPts, worldTangents, projMatrix);
...
if PARAMS.RESTRICT_TO_FREE_BOUNDARY == 1 && ~isempty(labels) && numel(labels) == size(uv, 1)
    keep = keep & strcmp(labels, "free");
end

uv = uv(keep, :);
theta = theta(keep);
fprintf('  %d boundary points selected for edge-support evaluation (view %d)\n', size(uv, 1), viewIdx0);
...
% ---- display ----
fig = figure('Name', sprintf('%s - view %d - edge support', PARAMS.MESH_NAME, viewIdx0), ...
...
hSupported = [];
if any(supported)
    hSupported = scatter(ax, uv(supported, 1), uv(supported, 2), 10, [0 0.45 0.9], 'filled', 'DisplayName', 'Boundary: supported');
end

hUnsupported = [];
if any(~supported)
    hUnsupported = scatter(ax, uv(~supported, 1), uv(~supported, 2), 10, [1 0 0], 'filled', 'DisplayName', 'Boundary: NOT supported');
end
...
boundaryDesc = 'boundary';
if PARAMS.RESTRICT_TO_FREE_BOUNDARY == 1
    boundaryDesc = 'free boundary';
end
title(ax, sprintf('%s — view %d — %d/%d %s pts supported (%.0f%%)', ...
      char(PARAMS.MESH_NAME), viewIdx0, nSupported, nPts, boundaryDesc, 100 * nSupported / max(1, nPts)), ...
...
if PARAMS.SAVE_FIGURE == 1
    ...
    savePath = fullfile(pwd, 'tmp', sprintf('%s_view%02d_edge_support.png', safeName, viewIdx0));
    try
        saveas(fig, savePath);
